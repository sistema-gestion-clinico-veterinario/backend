...
$plan = Join-Path $suiteRoot "plans\vargasvet-read-load.jmx"
...
$output = Join-Path $suiteRoot "results\load-200-$stamp"
$report = Join-Path $output "html"
New-Item -ItemType Directory -Force -Path $output | Out-Null

$jtl = Join-Path $output "results.jtl"
& $jmeter -n -t $plan -q $PropertiesFile -Jusers=200 -Jramp_seconds=300 -Jduration_seconds=900 -Jloops=-1 -l $jtl -j (Join-Path $output "jmeter.log") -e -o $report
if ($LASTEXITCODE -ne 0) { throw "La prueba de 200 usuarios falló. Revisa $output" }
...
Write-Host "Prueba de 200 usuarios terminada: $report\index.html"
