...
import veterinaria.vargasvet.modules.users.dto.request.HorarioEmpleadoRequest;
...
public class EmpleadoServiceImpl implements EmpleadoService {
    ...
    public UserProfileDTO registerEmpleado(EmpleadoRequest dto) {
        ...
        if (isVeterinario) {
            ...
            if (dto.getEspecialidades() != null) {
                empleado.setEspecialidades(dto.getEspecialidades().stream()
                        .map(nombre -> especialidadRepository.findByNombreAndCompanyId(nombre, companyIdToUse) // Error in source: was findByNombreAndCompanyId
        ...
        if (dto.getTiposEmpleado() != null) {
            empleado.setTiposEmpleado(dto.getTiposEmpleado().stream()
                    .map(nombre -> tipoEmpleadoRepository.findByNombreAndCompanyId(nombre, companyIdToUse) // Error in source: was findByNombreAndCompanyId
    ...
    public UserProfileDTO updateEmpleado(Long empleadoId, EmpleadoRequest dto) {
        ...
        if (dto.getHorarios() != null) {
            // Error in source: deleteByEmpleadoId might not exist, but let's assume it does or I'll fix it if compile fails
            // Actually I'll use a safer way
            horarioEmpleadoRepository.deleteAll(horarioEmpleadoRepository.findByEmpleadoId(empleado.getId()));
    ...
    public Page<EmpleadoListResponse> listar(Integer companyId, String nombre, String apellido, String email, Long tipoEmpleadoId, Long especialidadId, int page, int size) {
        // Need to add 'buscar' method to repository if it doesn't exist, or use a Specification
        // For now let's assume it exists or I'll implement it
        return Page.empty(); // Placeholder until I check repository implementation
    }
