...
public class UsuarioPrincipal implements UserDetails {
    ...
    public static UsuarioPrincipal create(Usuario usuario) {
        ...
        if (usuario.getRoles() != null) {
            for (veterinaria.vargasvet.modules.users.domain.entity.Role role : usuario.getRoles()) {
                authorities.add(new SimpleGrantedAuthority(role.getName()));
                if (role.getPermissions() != null) {
                    for (veterinaria.vargasvet.modules.users.domain.entity.Permission perm : role.getPermissions()) {
